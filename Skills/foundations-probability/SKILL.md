...
- **Negative Binomial parameterization varies**: NumPy, SciPy, and R all parameterize the Negative Binomial differently. Always check the documentation when switching between libraries.
- **Independence is a strong assumption**: When you sum up probability rules with "and" or "or," make sure the events are actually independent. In biology, gene expression levels are correlated — treating them as independent leads to incorrect inference.
